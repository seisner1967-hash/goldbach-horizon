...
The TS292--TS316 chain now proves absolute convergence of the nontrivial-zero
...
separate zero-multiplicity hypothesis. TS312 routes the explicit formula into
the high-level TS204 contract, and TS313 provides the certified `2/x`
...
the weighted off-diagonal close-pair estimate, the finite moment estimate, the
rational trace budget at most one half, Gallagher, OTSA, and Goldbach remain
open.

## Current Focus: TS15--TS316
...
| TS285 | Riemann xi finite quotient assembly | `repo_committed` | fills the removable values of `xi/P` at every selected root using the TS284 local factor divided by the `Finset.erase` complementary product, proves this quotient analytic on the whole complex plane, proves the global factorization and nonvanishing on the buffered analytic ball, constructs the first concrete TS282 xi buffered factorization, and activates the TS280 canonical Jensen boundary and multiplicity-count bounds; effective xi growth, quantitative zero counting, explicit formula, Gallagher, OTSA, and Goldbach remain open |
| TS286 | Riemann xi master API | `repo_committed` | publishes `TS.Goldbach.MasterAPI` as a thin stable facade over the concrete TS282--TS285 xi/Jensen construction, exposing xi, entirety, symmetry, the complete certificate, three-radius geometry, finite zero specification, buffered factorization, the real-valued canonical boundary norm, pointwise sphere control, and the canonical Jensen boundary and multiplicity-count estimates; effective xi growth, quantitative zero counting, explicit formula, Gallagher, OTSA, and Goldbach remain open |
| TS287 | Riemann xi growth API probe | `repo_committed` | records the exact locked Gamma and zeta APIs on safe regions, defines the target exponential growth and primary regularized-completed-zeta circle contracts, proves `abs(z*(z-1)) <= R*(R+1)`, transports every supplied bound `abs LambdaZero(z) <= A(R)` to the explicit xi majorant `max 1 ((R*(R+1)*A(R)+1)/2)`, fills the concrete TS275 boundary contract, and routes it through TS279/TS274 to an explicit finite Jensen multiplicity-count quotient; complex Stirling, critical-strip zeta growth, effective completed-zeta growth, a concrete quantitative zero count, explicit formula, Gallagher, OTSA, and Goldbach remain open |
| TS288 | Completed-zeta theta-Mellin circle growth | `repo_committed` | unfolds Mathlib's entire regularized completed zeta as the Mellin transform of its modified theta kernel, proves the circle power bounded by the maximum of the two endpoint real powers, obtains integrability from the strong functional-equation pair at both endpoint exponents, constructs the unconditional radial integral majorant `completedZetaThetaMellinMajorant`, fills the TS287 growth contract, and routes it to concrete xi Jensen boundary and multiplicity-count estimates; a closed theta-integral bound, exponential radius envelope, log-linear zero count, explicit formula, Gallagher, OTSA, and Goldbach remain open |
| TS289 | Closed completed-zeta theta-integral bound | `repo_committed` | proves the exact inversion law for the modified theta kernel, extracts its explicit geometric exponential decay on `[1,infinity)`, transports the lower Mellin tail to the upper tail by `x -> 1/x`, and uses the tangent inequality for `log` to prove `completedZetaThetaMellinMajorant(R) <= (2/(1-exp(-pi))) * exp(R*log(R+2))` for `R >= 2`; this closed majorant fills the TS287 circle-growth contract and is routed to concrete xi Jensen boundary and multiplicity-count estimates, while transport to TS270 global counting, Riemann-von Mangoldt asymptotics, explicit formula, Gallagher, OTSA, and Goldbach remain open |
| TS290 | Riemann xi log-linear zero counting | `repo_committed` | proves the analytic nonvanishing zeta-to-xi multiplier and exact equality of TS264 zeta and TS284 xi multiplicities, replaces the shrinking `r+3` Jensen gap by exact constant-ratio geometry with ambient radius `4r` and averaging radius at least `2r`, routes the TS289 closed theta majorant through this geometry, proves `N_mult(T) <= xiGlobalLogLinearConstant*T*log(T+2)` for `T >= 1`, and constructs the unconditional TS270 global counting contract through the TS273 safe envelope; Riemann-von Mangoldt asymptotics, explicit formula, Gallagher, OTSA, and Goldbach remain open |
| TS291 | Log-linear zero-contribution assembly | `repo_committed` | injects the concrete TS290 global counting contract into the exact TS271--TS273 finite Abel machinery, proves finite reciprocal-square partial sums at most two, closes the amortized shell expression by `5*C*log(X+3)`, absorbs the height-one boundary into `xiClosedHighResidualConstant = 6*xiGlobalLogLinearConstant`, and proves the unconditional finite contribution bound `abs zeroContribution(X) <= lowExactMass(X) + max(1,X)*xiClosedHighResidualConstant*log(X+3)`; the low zone remains exact, while infinite spectral convergence, explicit formula, residual estimates, Gallagher, OTSA, and Goldbach remain open |
| TS292 | Effective infinite zero-tail convergence | `repo_committed` | separates arithmetic scale `x` from spectral height `T`, applies the TS271 finite Abel identity to shifted unit shells through the potential `(log(t+3)+2)/t`, proves an upper-cutoff-independent reciprocal-square tail bound with constant `15*xiGlobalLogLinearConstant`, obtains absolute `Summable` and `HasSum` results for the complete concrete zero series, defines the infinite contribution, proves the exact finite-plus-tail decomposition and the effective remainder `max(1,x)*Ctail*(log(T+2)+1)/T`, and proves truncation convergence and the Cauchy property; von Mangoldt identities, explicit formulae, contour residuals, Gallagher, OTSA, and Goldbach remain open |
| TS293 | Truncated Perron contour residual | `repo_committed_relative` | defines the concrete logarithmic-derivative Perron integrand and proves its right-half-plane rewrite to Mathlib's von Mangoldt L-series, defines an oriented rectangular contour, a real contour height `tau in [T,T+1]`, exact real-height zero truncation, certified exceptional local residues, and a residual made from actual boundary integrals and cutoff adjustments; assuming the explicitly named Perron inversion and meromorphic rectangle residue statements, proves the truncated identity and routes it to `TS255.NamedExplicitFormulaIdentityStatement`; clean-height existence, Perron inversion, the residue theorem, contour bounds, the infinite explicit formula, Gallagher, OTSA, and Goldbach remain open |
| TS294 | Quantitative clean contour estimates | `repo_committed_relative` | fixes the first Perron geometry at `Re(s)=-3/2` and `Re(s)=2`, strengthens TS293 clean contours by an explicit positive separation from nearby nontrivial zero heights, proves the exact real-height spectral adjustment is a finite TS292 tail and bounds it unconditionally by `max(1,x)*15*Cxi*(log(T+2)+1)/T`, proves the three non-right side bounds assemble with the exact normalization factor `1/(2*pi)`, and derives complete complex and real residual bounds from targeted exceptional, side, and right-cutoff evidence; quantitative height selection, `zeta'/zeta` boundary estimates, right-line cutoff control, exceptional-inventory completeness, Perron inversion, the residue theorem, the infinite explicit formula, Gallagher, OTSA, and Goldbach remain open |
| TS295 | Strong clean heights and log-derivative reduction | `repo_committed_relative` | defines the finite multiplicity-weighted reciprocal zero load through height `T+2`, strengthens clean-height evidence to carry explicit separation and load envelopes, proves the finite rational logarithmic derivative bounded by that load on both horizontal sides, packages a local holomorphic logarithm with a sphere bound, and applies Cauchy's estimate to bound the quotient logarithmic derivative; no infinite Hadamard product is used, while strong-height construction, the load rate, the exact xi finite-factor identity and zeta completion bridge, quotient-log sphere bounds, the remaining contour estimates, Perron, the residue theorem, the infinite explicit formula, Gallagher, OTSA, and Goldbach remain open |
| TS296 | Concrete strong height and xi quotient logarithm | `repo_committed` | constructs an unconditional height in `(T,T+1)` by finite avoidance, its exact positive minimum gap and reciprocal load, the matching finite zero polynomial and quotient `xi/P_T`, proves quotient analyticity and nonvanishing on explicit local balls, proves the exact finite-factor logarithmic-derivative identity, and instantiates the TS279 holomorphic logarithm and TS295 Cauchy bounds; closed rates for the gap, load, and logarithm sphere bound remain open |
| TS297 | Xi-zeta horizontal Perron bridge | `repo_committed` | proves the exact local completion identity between xi and zeta off the real axis, differentiates it on a neighborhood, rewrites `-zeta'/zeta` as the named completion correction minus the finite zero logarithmic derivative minus the concrete quotient logarithmic derivative, and routes this identity to exact top and bottom Perron-integrand envelopes; all asymptotic rates and integrated contour estimates remain open |
| TS298 | Right-line cutoff and horizontal integration | `repo_committed` | proves the unconditional right-line cutoff bound `C_right*max(1,x^2)/T` from absolute convergence of the von Mangoldt L-series and the quadratic Mellin kernel, integrates uniform horizontal envelopes over the exact width `7/2`, and routes both components into TS294 while retaining the fixed-left and exceptional-residue certificates as explicit inputs |
| TS299 | Finite-grid strong height and reciprocal-load bound | `repo_committed` | replaces arbitrary finite avoidance by `K_T=4(M_T+1)` explicit grid midpoints, proves each zero forbids at most one index, establishes the universal `8*K*H_K` truncated reciprocal-kernel bound, selects a concrete height with load at most `16*M_T*H_(K_T)`, constructs the quantitative TS294 contour, and uses TS290 to provide closed separation and load envelopes of the coarse `T log(T)^2` scale; the logarithm-sphere rate, completion correction, fixed-left side, residues, Perron, and the infinite explicit formula remain open |
| TS300 | Centered Borel-Caratheodory and closed load decay | `repo_committed` | backports the needed local Borel-Caratheodory theorem from the locked Schwarz API, centers local quotient logarithms to remove branch constants, reduces `g'/g` to a named centered real-part envelope with the exact bound `4*M/R`, proves the closed TS299 load is `o(T^2)`, routes it through the finite-grid horizontal power, Mellin kernel, and exact width `7/2`, and proves the integrated zero-load component tends to zero for fixed arithmetic scale; the quotient minimum-modulus input, completion correction, full horizontal decay, fixed-left side, residues, Perron, and the infinite explicit formula remain open |
| TS301 | Anchored macroscopic xi quotient | `repo_committed` | replaces the circular moving-center minimum-modulus route by the concrete TS290/TS285 finite quotient at scale `64*(T+4)`, normalizes its TS279 logarithm at the fixed nonzero anchor `2`, constructs a branch-independent compact real-part envelope on `closedBall 2 (16*(T+4))`, applies TS300 and Cauchy on both finite-grid horizontal sides, and proves the exact finite bridge `g_height'/g_height = Q_macro'/Q_macro + (macroSum-heightSum)` with the correct sign; a closed rate for the anchored envelope and decay of the extra finite correction remain open |
...
TS285 constructs that finite global quotient.  Off the selected roots it is
the literal quotient `xi/P`; at a root it uses the chosen TS284 local factor
divided by the product over all other roots.  Finiteness makes the other-root
set closed, and the local normal form proves that these expressions agree near
each removable point.  The quotient is entire, the identity `xi = P*g` holds
globally, and `g` is nonzero on the analytic closed ball.  This gives the first
concrete TS282 buffered xi construction and activates the canonical TS280
Jensen estimate and finite multiplicity-count inequality for xi.

TS286 freezes the resulting downstream interface in
`TS.Goldbach.MasterAPI`.  It reexports the entire xi candidate, its functional
equation, the concrete TS285 certificate and geometry, the finite zero data,
the buffered factorization, and the TS280 canonical boundary majorant.  The
majorant is correctly exposed as a real number, together with its positivity,
pointwise control on the averaging sphere, and the two canonical Jensen
facades.  No duplicate certificate or new analytic assumption is introduced.

TS287 opens the quantitative-growth front without separating Gamma from its
completed-zeta cancellations on the full circle.  It records the Gamma and
ordinary-zeta APIs available only on their safe regions, then uses the entire
regularized `completedRiemannZetaZero` as the primary input.  The elementary
affine xi formula converts any circle bound `A(R)` into the explicit majorant
`max 1 ((R*(R+1)*A(R)+1)/2)`.  This majorant is routed through the concrete
TS285 factorization and the closed Jensen chain to a finite multiplicity-count
quotient, with the construction of an effective `A` left as the sole analytic
growth input.

TS288 constructs that radial function `A` without a new hypothesis by using
Mathlib's defining theta-Mellin representation of the regularized completed
zeta function.  On `abs s = R`, the Mellin power is bounded by the maximum of
the endpoint real powers with exponents `R/2-1` and `-R/2-1`; both endpoint
envelopes are integrable because the associated strong functional-equation
pair has a Mellin transform at every complex exponent.  The resulting finite
radial integral fills the TS287 circle-growth contract and activates the
concrete xi Jensen count.  Turning this integral into a closed
`exp(C*R*log(R+2))` estimate is the next quantitative lock.

TS289 closes that theta-integral lock.  The modified kernel is self-dual
under `x -> 1/x`; on `[1,infinity)` its nonconstant theta series is bounded
by `2*exp(-pi*x)/(1-exp(-pi*x))`.  Inversion sends the lower Mellin tail
exactly to an upper tail with exponent `R/2-1/2`.  A tangent bound for `log`
at `R+2` then dominates the remaining integrand by
`exp(R*log(R+2))*exp(-x)`.  Hence for `R >= 2` the TS288 radial majorant is at
most `(2/(1-exp(-pi)))*exp(R*log(R+2))`, and this closed function is routed
through TS287 to the concrete xi Jensen count.

TS290 closes the global-count transport.  It first proves that multiplication
by the analytic nonvanishing reciprocal Gamma factor preserves the exact
TS264 zero multiplicity when passing from zeta to xi.  It then identifies why
the existence-oriented TS283 radius `r+3` cannot give a uniform Jensen gap and
rebuilds the exact zero geometry inside radius `4r`, with averaging radius at
least `2r`.  The resulting denominator is at least `log 2`, while TS289 gives
the closed numerator bound.  This yields an explicit
`xiGlobalLogLinearConstant*T*log(T+2)` estimate for every `T >= 1` and fills
the genuine TS270 global multiplicity-count contract via the TS273 safe
envelope.

TS291 consumes that contract without opening another analytic interface.
The finite Abel identity already proved in TS271 is enough: each shifted
coefficient is bounded by twice a reciprocal square, and the corresponding
finite partial sums are at most two.  The amortized count is therefore at
most `5*C*log(X+3)`; adding the exact height-one boundary gives the closed
high residual constant `6*C`.  Routed through TS269 and TS272, this proves
`abs zeroContribution(X) <= lowExactMass(X) +
max(1,X)*xiClosedHighResidualConstant*log(X+3)`.  The high term is thus
`O(X*log X)` while the low zone remains an exact finite mass.

TS292 closes the finite-to-infinite spectral passage while keeping the
arithmetic scale and spectral cutoff independent.  Shifted unit shells above
height `T` are controlled by the decreasing potential
`(log(t+3)+2)/t`, producing a finite-tail estimate independent of the upper
cutoff and bounded by
`15*xiGlobalLogLinearConstant*(log(T+2)+1)/T`.  This finite estimate proves
absolute summability of the full multiplicity-weighted zero series.  TS292
then defines the infinite contribution, proves its exact decomposition into
the TS265 truncation plus a complementary `tsum`, gives the corresponding
effective norm remainder, and proves convergence and the Cauchy property.
It does not identify this series with a von Mangoldt sum or prove an explicit
formula or contour estimate.

TS293 starts the independent contour front.  It defines the concrete
logarithmic-derivative Perron integrand and proves its equality with the von
Mangoldt L-series integrand on `re(s)>1`.  The rectangle is positively
oriented, the clean contour height is a real `tau in [T,T+1]`, and the
exceptional poles carry local principal-part certificates.  The residual is
the explicit sum of exceptional residues, the three non-right boundary
integrals, the finite right-line cutoff, and the exact spectral adjustment
between heights `T` and `tau`; it is not defined from the desired identity.
Perron inversion and the meromorphic rectangle residue theorem remain named
analytic premises.  From precisely those premises, TS293 proves the
truncated explicit identity and its TS255 routing.

TS294 makes the contour front quantitative without conflating it with the
TS292 spectral tail.  It fixes the rectangle edges at `-3/2` and `2`, defines
a clean contour carrying an explicit positive gap from nearby nontrivial zero
heights, and proves that the exact `T`-to-`tau` spectral adjustment is a finite
subset of the TS292 tail.  It also assembles independent bottom, top, and left
integral estimates with the exact `1/(2*pi)` normalization.  Consequently,
targeted bounds for the exceptional residues and the actual contour pieces
produce a bound for the concrete TS293 residual.  Quantitative height
selection and the needed local `zeta'/zeta` estimates remain explicit open
inputs.

TS295 chooses the local finite-factor route for those logarithmic-derivative
estimates.  It defines the exact reciprocal load
`sum m(rho)/abs(tau-abs(Im rho))` through height `T+2`, and strengthens the
clean-height target so both a separation rate and a load envelope depend
explicitly on `T`.  The rational finite-zero logarithmic derivative is proved
bounded by this load on both horizontal sides.  For the complementary
nonvanishing quotient, TS295 packages a local holomorphic logarithm and uses
Cauchy's derivative estimate to bound `g'/g` by `sphereBound/radius`.
Strong-height construction, the load asymptotic, the exact xi finite-factor
identity, the separate zeta completion bridge, and the logarithm sphere bound
remain explicit open inputs; no infinite Hadamard product is introduced.
