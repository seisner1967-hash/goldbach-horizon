...
| TS190 | Triangle spline critical-line amplitude | `repo_committed` | specializes the TS189 logarithmic amplitude to `c = 1 / 2`, proving nonnegativity, zero branch after `(X : Real) <= exp u`, and affine branch `(1 - exp u / X) * exp (u / 2)` on the support without claiming RH or measure transport |
...
critical-line shift `c = 1 / 2`.  It defines
`triangleSplineCriticalAmplitude X u` as the TS189 amplitude with scale
`(X : Real)` and shift `1 / 2`, proves nonnegativity, proves that the amplitude
vanishes once `(X : Real) <= exp u`, and proves that on the support side
`exp u <= (X : Real)` it is exactly
`(1 - exp u / (X : Real)) * exp (u / 2)`.  This is only an algebraic
specialization of the profile; TS190 does not prove the Riemann hypothesis,
does not prove the Wall 0 measure transport, does not prove the explicit
formula, does not prove Plancherel, and does not prove Goldbach.
...
above is specifically the audited `TS/Goldbach/Strong/TS15`--`TS190` layer.
